...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent':
              'CORGIS Food Access library for educational purposes'}
...
def get_records(test=False):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM food_access LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM food_access".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
